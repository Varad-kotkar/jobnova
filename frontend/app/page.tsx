...
export default async function HomePage(props: HomePageProps) {
  ...
  const trustedCompanies = ["Google", "Stripe", "Microsoft", "Amazon", "NVIDIA", "Razorpay", "Swiggy", "Zomato", "Zoho", "Vercel", "Figma", "Notion"];

  let initialHomeJobs = null;
  try {
    initialHomeJobs = await getHomeJobs();
  } catch (err) {
    console.warn("HomePage server pre-fetch error:", err);
  }
    ...
    <div className="space-y-16 pb-16 bg-white">
      ...
      <section className="border-b border-gray-100 bg-gray-50/50 py-16 px-4 sm:px-6">
        <div className="mx-auto max-w-4xl text-center space-y-6">
          <div className="inline-flex items-center gap-2 rounded-full border border-blue-200 bg-blue-50 px-3.5 py-1 text-xs font-semibold text-blue-700">
            ...
            JobNova • Student & Fresher Job Discovery Platform
          </div>

          <h1 className="text-4xl sm:text-5xl font-extrabold tracking-tight text-gray-900 leading-tight">
            Discover Curated <span className="text-blue-600">Tech Roles, Internships & Fresher Jobs</span>
          </h1>

          <p className="max-w-2xl mx-auto text-base text-gray-600 font-normal leading-relaxed">
            Curated engineering listings, remote opportunities, and entry-level positions indexed from verified ATS portals. Updated continuously for job seekers.
          </p>
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6">
        <div className="rounded-3xl border border-blue-100 bg-gradient-to-r from-blue-50/80 via-indigo-50/50 to-purple-50/80 p-8 shadow-subtle grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
          <div>
            ...
            <p className="text-xs font-bold text-blue-700/80 mt-1 uppercase tracking-wide">Live Tech Roles</p>
          </div>
          <div>
          ...
          <div>
            ...
            <p className="text-xs font-bold text-emerald-700/80 mt-1 uppercase tracking-wide">Verified Companies</p>
          </div>
          <div>
            <p className="text-2xl sm:text-3xl font-black text-purple-900">{statsData.total_applications}+</p>
            <p className="text-xs font-bold text-purple-700/80 mt-1 uppercase tracking-wide">Applications Sent</p>
          </div>
        </div>
      </section>

      {/* Curated Job Discovery Sections */}
      <CuratedHomeSections initialData={initialHomeJobs} />

      {/* AI Features & Career Guidance Spotlight */}
      <section className="mx-auto max-w-7xl px-4 sm:px-6">
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 space-y-6">
        <div className="flex items-center justify-between border-b border-gray-100 pb-4">
          <div>
            <h2 className="text-xl font-bold text-gray-900">Explore Tech Categories</h2>
